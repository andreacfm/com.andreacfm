...
<!---- getScaledDimensions --->
<!--------------------------------------------------------------------------------------------------------------------->
	<cffunction name="getScaledDimensions" output="false" returntype="com.andreacfm.core.object">
		<cfargument name="imgRoot" required="true"/><!--- absolute path to the image --->
		<cfargument name="width" required="false" default="" />
		<cfargument name="height" required="false" default="" />
		
		<cfset var result = createObject('component','com.andreacfm.util.message').init() />
		<cfset var img = "" />
		<cfset var dimensions = {} />
		
		<cftry>
			<cfset img = imageNew(arguments.imgRoot) />
			<cfset imageScaleToFit(img,arguments.width,arguments.height,'highestPerformance') />
			<cfset dimensions.height = img.height />
			<cfset dimensions.width = img.width />
			<cfcatch type="any">
				<cfset result.setStatus(false) />
				<cfset result.setType('error') />
				<cfset result.setText(#cfcatch.ExtendedInfo#) />
				<cfreturn result />
			</cfcatch>
		</cftry>
		
		<cfset result.setData(dimensions) />
