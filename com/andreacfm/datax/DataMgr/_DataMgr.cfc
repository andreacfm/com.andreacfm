...
<cffunction name="getRecordsSQL" access="public" returntype="array" output="no" hint="I get the SQL to get a recordset based on the data given.">
	...
	<!--- SEB 2009-10-09: No longer needed in faver of getOrderBySQL --->
	...
	<cfif Len(arguments.function) AND NOT Len(arguments.fieldlist)>
		<cfset arguments.noorder = true>
	</cfif>
	...
	<!--- query order --->
	<cfif ArrayLen(aOrderBySQL) AND NOT ( StructKeyExists(arguments,"noorder") AND arguments.noorder EQ true )>
...
<cffunction name="applyListRelations" access="private" returntype="query" output="no">
	<cfargument name="tablename" type="string" required="yes">
...
<cffunction name="useField" access="private" returntype="boolean" output="no" hint="I check to see if the given field should be used in the SQL statement.">
	...
	<cfif
		...
		AND	isOfType(Struct[Field.ColumnName],getEffectiveFieldDataType(Field,true))
	>
		<cfset result = true>
	</cfif>
