<cfcomponent 
	displayname="Json" 
	hint="Json Converter" 
	extends="cfops.core.object" 
	output="false">

	<!---	constructor	--->		
	<cffunction name="init" description="initialize the object" output="false" returntype="cfops.core.object">	
		<cfreturn this/>	
	</cffunction>

	<!---	json	--->
	<cffunction name="json" output="false" returntype="any" description="Convert data in array of struct and serialize if required">
		<cfargument name="obj" required="true">
		<cfargument name="serialize" required="false" type="boolean" default="false" />				
		<cfscript>
			var result = arrayNew(1);
			if(isQuery(arguments.obj)){
				result = queryToArray(arguments.obj); 
			}else if(isArray(arguments.obj)){
				if(arrayIsEmpty(arguments.obj)){
					result = arguments.obj;
				}else{
					for(i = 1 ; i <= arraylen(arguments.obj) ; i ++){
						result[i] = arguments.obj[i].getmemento(); 				
					}
				}
			};
